...
export function resolveElement(target: DomTarget): HTMLElement {
    if (typeof target === 'string') {
        ...
        if (!element) {
            throw new Error(`[domsnap]: No element found matching selector "${target}".`);
        }
        return element;
    }
    ...
    throw new Error('[domsnap]: Target must be a valid CSS selector string or an HTMLElement.');
}
...
export function validateElementDimensions(element: HTMLElement): { width: number; height: number } {
    ...
    if (width <= 0 || height <= 0) {
        throw new Error('[domsnap]: Cannot capture element with width or height of 0. Ensure the element is visible in the DOM.');
    }
...
export function validateOptions(options: { scale?: number; quality?: number; format?: string }): void {
    if (options.scale !== undefined && (typeof options.scale !== 'number' || options.scale <= 0)) {
        throw new Error('[domsnap]: Scale option must be a positive number.');
    }

    if (options.quality !== undefined && (typeof options.quality !== 'number' || options.quality < 0 || options.quality > 1)) {
        throw new Error('[domsnap]: Quality option must be a number between 0 and 1.');
    }

    if (options.format !== undefined && options.format !== 'png' && options.format !== 'jpeg' && options.format !== 'webp') {
        throw new Error('[domsnap]: Format option must be "png", "jpeg", or "webp".');
    }
}
